...
OCSResponse OCServo::setZeroCorrection(int correction) {
    byte params[2] = {
        lowByte(correction),
        highByte(correction)
    };
    return this->ocsWrite(OCS_ZERO_CORRECTION, 2, params);
}

OCSResponse OCServo::setLockSign(byte lock) {
    OCSResponse resp =  this->ocsWrite(OCS_LOCK_SIGN, 1, &lock);
    return resp;
}

OCSResponse OCServo::setCWDeadband(byte deadband) {
    OCSResponse resp =  this->ocsWrite(OCS_CW_DEADBAND, 1, &deadband);
    return resp;
}

OCSResponse OCServo::setCCWDeadband(byte deadband) {
    OCSResponse resp =  this->ocsWrite(OCS_CCW_DEADBAND, 1, &deadband);
    return resp;
}
